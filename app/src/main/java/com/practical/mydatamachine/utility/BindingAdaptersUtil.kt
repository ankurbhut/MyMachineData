...
import com.practical.mydatamachine.shared.extension.changeItemAnimatorDuration
import com.practical.mydatamachine.shared.extension.removeItemAnimator
...
//        }
    ...
    fun setFloatingEffect(view: View, startEffect: Boolean) {
        if (startEffect) view.startAnimation(
            AnimationUtils.loadAnimation(
                view.context, R.anim.floacting_effect
            )
        )
    }

    @JvmStatic
    @BindingAdapter(value = ["android:setPulseRaysAnim"], requireAll = false)
    fun setPulseRaysAnim(view: View, duration: Long = 0) {
        val anim = AnimationUtils.loadAnimation(view.context, R.anim.pulse_rays_anim)
        if (duration > 0) {
    ...
    @JvmStatic
    @BindingAdapter(value = ["android:setShakeEffect"], requireAll = false)
    fun setShakeEffect(view: View, startEffect: Boolean) {
        if (startEffect) view.startAnimation(
            AnimationUtils.loadAnimation(
                view.context, R.anim.shaking_effect
            )
        )
    }
    ...
    fun loadMediaWithProgress(
        ...
        if (url!! == "dummy_amazon") {
            ...
            getRequestOption(placeHolder, imageSize).let {
                Glide.with(Controller.instance.applicationContext).asDrawable().load(url)
    ...
    @JvmStatic
    @BindingAdapter(value = ["android:setLockUnlockHintAnim"], requireAll = false)
    fun setLockUnlockHintAnim(view: View, startEffect: Boolean) {
        if (startEffect) view.startAnimation(
            AnimationUtils.loadAnimation(
                view.context, R.anim.lock_unlock_hint_anim
            )
        )
    }
}